...
__global__ void generate_hexagonal_grid_of_rays_kernel(
    ...
    const int gi = i % grid_width;
    const int gj = i / grid_width;
...
void HexagonalGridRayBatchCoordinator::generate_rays(
    ...
    generate_hexagonal_grid_of_rays_kernel<<<n_blocks_linear(ray_batch.size), n_threads_linear, 0, stream>>>(
        ray_batch.size,
        ray_batch.stride,
        grid_dims.x,
        grid_offset,
...
__global__ void copy_packed_rgba_hexagonal_grid_kernel(
    ...
    if (i_out >= n_pixels_out) {
        return;
    }

    // first we must get the output x,y indices
    int y = divide(i_out, W); // (i_out / W)
    int x = i_out - W * y;    // (i_out % W)
...
void HexagonalGridRayBatchCoordinator::copy_packed(
    ...
    copy_packed_rgba_hexagonal_grid_kernel<<<n_blocks_linear(n_pixels), n_threads_linear, 0, stream>>>(
        n_pixels,
        stride,
        output_size.x,
        grid_dims.x,
