...
struct BoundingBox {
    ...
    inline NRC_HOST_DEVICE float3 size_xyz() const {
        return {
            size_x(),
            size_y(),
            size_z()
    ...
    inline NRC_HOST_DEVICE float3 pos_to_unit(const float3& p) const {
        return {
            pos_to_unit_x(p.x),
            pos_to_unit_y(p.y),
            pos_to_unit_z(p.z)
    ...
    inline NRC_HOST_DEVICE float3 unit_to_pos(const float3& p) const {
        return {
            unit_to_pos_x(p.x),
            unit_to_pos_y(p.y),
            unit_to_pos_z(p.z)
    ...
    inline NRC_HOST_DEVICE float3 center() const {
        return {
            0.5f * (min_x + max_x),
            0.5f * (min_y + max_y),
            0.5f * (min_z + max_z)
